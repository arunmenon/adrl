...
**Finding.** The design's registry sketch (§5.4) assumed `tool_call_reliability.claude_code = 0.87` for the local rung — i.e. frequent dialect failures, the premise of scenario S5. That does **not hold** for `qwen2.5:7b-instruct-q4_K_M` on Claude Code: measured reliability on these edit-heavy sessions is ≈1.0. This is exactly what the (model, harness)-keyed registry exists to catch — measuring instead of assuming. S5 remains a real failure mode for *weaker* local models (base models, older GGUFs, 3B-class), but this instruct-tuned 7B does not exhibit it.
